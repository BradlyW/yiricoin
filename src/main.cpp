...
#if defined(NDEBUG)
# error "Yiricoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xec38228e917d57b70242e423bed2a78d9049f8e9ce3d3ba3ab6fc9347ea1c114");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yiricoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yiricoin Signed Message:\n";
...
{
    // Yiricoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Yiricoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Yiricoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Yiricoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Yiricoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yiricoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd67450aee8e3b740c5550185b83d215ed5ccb801befee66084ecfca5a8bdd90f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CEO Of YiriCorp creates the best coin ever!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040a0eb4ff553bfbc122313278d1b4bf4ee1f7f02269d266cc4e2f7cc2f9f94d3c4735a3aa611b0e3033d52a4fae5f79a586ba7aadaf6192b0877c919916209c8d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516242331;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087979725;

        if (fTestNet)
        {
            block.nTime    = 1516242331;
            block.nNonce   = 386510557;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xeb12021dc172baa13e7f0b0cd1b45094e6875e07696a5c1a3a4ac5c1bbb1bf29"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb3, 0xdc }; // Yiricoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YiricoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YiricoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YiricoinMiner(CWallet *pwallet)
{
    printf("YiricoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yiricoin-miner");
    ...
    try { loop {
        ...
        printf("Running YiricoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YiricoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YiricoinMiner, pwallet));
}
